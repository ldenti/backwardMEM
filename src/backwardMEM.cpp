...
void maximal_exact_matches(const tCST &cst1, const unsigned char *s2, tCST::size_type n2, tCST::size_type l, bool _4column, const vector<string> &refdescr, const vector<long> &startpos, long maxdescrlen, tCST::size_type p2_offset=0){
    ...
    /**
    std::cout << cst1.size() << std::endl;
    for (auto it=cst1.begin(); it!=cst1.end(); ++it) {
        if (it.visit() == 1) {  // node visited the first time
            auto v = *it;       // get the node by dereferencing the iterator
	    cout<<cst1.depth(v)<<"-["<<cst1.lb(v)<< ","<<cst1.rb(v)<<"]"<<endl;
        }
    }
    **/
	...
	for(path_type::iterator it=path.begin(); it!=path.end(); ++it){
		...
		lb = lb_; rb = rb_;
		//std::cout << c_ << " " << lb_ << " " << rb_ << std::endl;
		node_type p = cst1.parent( cst1.node(lb_, rb_-1) );
		//cout<<cst1.depth(p)<<"-["<<cst1.lb(p)<< ","<<cst1.rb(p)<<"]"<<endl;
	...
	if(0==c){
	    ...
	    //cout<<cst1.depth(p)<<"-["<<cst1.lb(p)<< ","<<cst1.rb(p)<<"]"<<endl;
	    c = cst1.depth(p); // SI DOVREBBE USARE DEPTH INVECE DI NODE_DEPTH E SENZA -1
	    //std::cout << c << std::endl;
	    i = cst1.lb(p);
	    j = cst1.rb(p)+1;
	}
